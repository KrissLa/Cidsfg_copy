...
document.querySelector('#video-preview').addEventListener('click', e => {
    ...
    const videoLink = e.target.getAttribute('data-video'),
        videoDiv = `<video controls autoplay>
                            <source src=${videoLink} type="video/mp4">
                    </video>`;
    e.target.parentNode.insertAdjacentHTML('afterbegin', videoDiv);
})

const setAboutBackground = (el) => {
    if (document.documentElement.clientWidth > 600) {
        el.style.backgroundImage = `url(${el.getAttribute('data-background')}`;
    }
}

setAboutBackground(document.querySelector('#about'));
setAboutBackground(document.querySelector('#factory'));
